...
class Text:
    ...
    def draw(self):
        if game.Screen.scene == self.scene and self.active:
...
class Button:
    ...
    def create_object(self):
        ...
        self.object.set_alpha(100)

        self.button_object = game.screen.blit(self.object, (-100, -100))
    ...
    def draw(self):
        if game.Screen.scene == self.scene and self.active:
            if self.button_object.collidepoint((game.mx, game.my)):
                self.object.set_alpha(150)
            else:
                self.object.set_alpha(100)
